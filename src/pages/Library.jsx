...
import { 
  Plus, Trash2, X, Search, Dumbbell, 
  Flame, Move, Edit3, AlertTriangle, ChevronLeft 
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 pb-40">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <Link to="/" className="text-slate-400 flex items-center gap-1 text-[10px] font-black uppercase tracking-widest mb-1">
            <ChevronLeft size={12}/> Back to Home
          </Link>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">Exercise Library</h1>
        </div>
        <Link 
          to="/add-exercise" 
      ...
      {/* Type Tabs */}
      <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar scroll-smooth mb-4" style={{ scrollbarWidth: 'none' }}>
      ...
      {/* Muscle Tabs */}
      <div className="flex gap-2 overflow-x-auto py-4 mb-6 no-scrollbar scroll-smooth border-b border-slate-200" style={{ scrollbarWidth: 'none' }}>
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="py-20 text-center animate-pulse text-slate-300 font-bold uppercase tracking-widest text-xs">Loading Library...</div>
          ...
          <div className="py-20 text-center bg-white rounded-[32px] border-2 border-dashed border-slate-100">
            ...
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">No exercises found</p>
          </div>
        ) : (
          filteredLibrary.map(ex => (
            <div key={ex._id} className="bg-white p-5 rounded-[28px] shadow-sm border border-slate-100 flex justify-between items-center group">
              <div className="flex items-center gap-4">
                <div className={`p-3 rounded-2xl ${ex.type === 'Warmup' ? 'text-amber-500 bg-amber-50' : ex.type === 'Stretching' ? 'text-blue-500 bg-blue-50' : 'text-emerald-500 bg-emerald-50'}`}>
              ...
              <div className="flex gap-2">
                <button onClick={() => setEditingExercise(ex)} className="p-3 bg-slate-50 rounded-xl text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 transition-all">
                  <Edit3 size={18}/>
                </button>
                <button onClick={() => setShowDeleteConfirm(ex._id)} className="p-3 bg-slate-50 rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all">
                  <Trash2 size={18}/>
                </button>
              </div>
            </div>
      ...
      {/* --- MODALS (Reused from ActiveWorkout) --- */}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[300] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-sm rounded-[40px] p-8 shadow-2xl animate-in zoom-in duration-200">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
                  {['Chest', 'Back', 'Shoulders', 'Biceps', 'Triceps', 'Legs', 'Abs', 'Full Body'].map((m) => (
                    <button key={m} onClick={() => setEditingExercise({...editingExercise, muscle: m})} className={`whitespace-nowrap px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider border transition-all ${editingExercise.muscle === m ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-white border-slate-100 text-slate-400'}`}>{m}</button>
                  ))}
                </div>
              </div>
            </div>
            <div className="flex gap-3 mt-8">
              ...
              <button onClick={updateLibraryItem} className="flex-1 py-4 bg-emerald-600 text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[300] flex items-center justify-center p-6 text-center">
          <div className="bg-white w-full max-w-sm rounded-[40px] p-8 shadow-2xl">
            ...
            <p className="text-slate-500 text-sm mb-8 leading-relaxed">This will remove it from your library forever.</p>
            <div className="flex gap-2">
